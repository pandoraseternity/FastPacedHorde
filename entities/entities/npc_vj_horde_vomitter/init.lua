...
self.RangeAttackExtraTimers = {1, 1.1}
self.RangeAttackReps = 2
end
end

function ENT:CustomRangeAttackCode_BeforeProjectileSpawn(projectile) 
if cvars.Number(self.CVar, 1) >= 3 && IsValid(self:GetEnemy()) then
self:SetAngles((self:GetEnemy():GetPos() +self:GetEnemy():GetAbsVelocity()*2 -self:GetPos()):Angle())
end
end
