...
PERK.Hooks.Horde_OnSetPerk = function(ply, perk)
    if SERVER and perk != "demolition_base" and perk == "warlock_base" then
		if perk == "demolition_base" then return end
        ply:Horde_SetMindRegenTick(0.25)
        ply:SetMaxArmor(0)
        if ply:HasWeapon("horde_astral_relic") == true then return end
		--PrintMessage( HUD_PRINTTALK, "warlock" )
        ...
        timer.Simple(0.01, function() if perk == "warlock_base" then
            if !ply:Alive() then return end
            if not ply:Horde_GetPerk("warlock_base") then return end
			--PrintMessage( HUD_PRINTTALK, "warlock2" )
