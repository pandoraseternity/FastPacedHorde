...
Melee attacks remove Nemesis/Transcendent mutation from enemies.]]
...
PERK.Hooks.Horde_OnPlayerDamage = function (ply, npc, bonus, hitgroup, dmginfo)
    if ply:Horde_GetPerk("psycho_disembowel") and HORDE:IsMeleeDamage(dmginfo) then
		...
		elseif npc:Horde_HasMutation("transcendent") then
            npc.Horde_Mutation["transcendent"] = nil
            npc.Horde_Mutation_Transc = nil	
        end
    end
end