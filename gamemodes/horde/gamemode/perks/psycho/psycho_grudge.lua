...
Remove 25 Armor upon melee damage]]
...
PERK.Hooks.Horde_OnPlayerCriticalCheck = function (ply, npc, bonus, hitgroup, dmginfo, crit_bonus)
    if ply:Horde_GetPerk("psycho_grudge") then
        crit_bonus.increase = crit_bonus.increase + 0.01 * (math.max(0, ply:Health() - 1 / ply:GetMaxHealth()))
    end
end

PERK.Hooks.Horde_PlayerMoveBonus = function (ply, bonus_walk, bonus_run)
    if ply:Horde_GetPerk("psycho_grudge") then
        local increase = math.min(0.5, math.max(0, (0.1 * (1 - ply:Health() / ply:GetMaxHealth()))) )
...
PERK.Hooks.Horde_OnPlayerDamagePost = function (ply, npc, bonus, hitgroup, dmginfo)
    if ply:Horde_GetPerk("psycho_grudge") and HORDE:IsMeleeDamage(dmginfo) then
