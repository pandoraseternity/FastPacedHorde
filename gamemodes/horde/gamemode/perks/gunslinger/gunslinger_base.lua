...
PERK.Hooks.Horde_UseActivePerk = function (ply)
    ...
    if (ent:IsValid() and ent:IsNPC()) or (ent:IsValid() and ent:IsNextBot()) and (not ent:GetNWEntity("HordeOwner"):IsValid()) then
        local range = 640000
        local delay = 5
        ...
        if ply:Horde_GetPerk("gunslinger_exorcism") then
            if ent.Horde_Mutation then
                ...
                if mutations_count > 0 then
                    ...
                    dmg:SetDamage(ent:GetMaxHealth() * 0.045 * mutations_count)
