...
local exp = {npc_grenade_frag = 100, npc_satchel = 100, npc_tripmine = 100, hl1_monster_tripmine = 100, arccw_thr_go_frag = 200, arccw_thr_horde_m67 = 200}
PERK.Hooks = {}
PERK.Hooks.EntityRemoved = function(ent)
    ...
    if SERVER and IsValid(owner)
        ...
        for i = 1, 3 do
            ...
            prop:Spawn()
            prop:Activate()
            timer.Simple(0, function() prop:GetPhysicsObject():AddVelocity(VectorRand() * 100 + Vector(0, 0, 150)) end)
            timer.Simple(0.5 + i * 0.1, function() if IsValid(prop) then
				...
				d:SetDamage(dmg)
				for k, npc in pairs(ents.FindInSphere(prop:GetPos(), 250)) do
