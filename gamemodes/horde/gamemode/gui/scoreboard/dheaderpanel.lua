...
function PANEL:Init()
	...
	self.m_HealthLabel = HORDE:EasyLabel(self.m_HeaderPanel, " ", "Content", color_white)
...
function PANEL:PerformLayout()
    ...
    self.m_KillsLabel:AlignRight(w * 0.20, 0)
	self.m_KillsLabel:CenterVertical()
    self.m_KillsLabel:SizeToContents()

    self.m_DeathsLabel:AlignRight(w * 0.15, 0)
	self.m_DeathsLabel:CenterVertical()
    self.m_DeathsLabel:SizeToContents()
	
    self.m_HealthLabel:AlignRight(w * 0.08, 0)
	self.m_HealthLabel:CenterVertical()
    self.m_HealthLabel:SizeToContents()

    self.m_PingLabel:AlignRight(w * 0.0075)
...
function PANEL:RefreshContents()
    ...
    self.m_HealthLabel:SetText( "Health" )
    self.m_HealthLabel:SizeToContents()
