...
function PANEL:Init()
    ...
    local update_text =[[
	Update Notes for the Server will now be posted here.
			
		-- Config Changes:
			< All dynamic difficulty changes will now start from Realism.
			< Weepers will now have a faster scream attack on Nightmare and above.
			< Fixed Rekindled Behemoth Homing projectiles.]]
    local mt = multlinetext(update_text, update_text_panel:GetWide() - 50, 'Content')
    update_text_panel.Paint = function ()
        draw.SimpleText("Server Update 0.0.1", 'LargeTitle', 50, 50, color_white, TEXT_ALIGN_LEFT, TEXT_ALIGN_CENTER)
    ...
    enemies_text_panel.Paint = function ()
        ...
        draw.SimpleText("Zombies with fast speed that come in hordes that are easy to dispose of. On nightmare and above, they will leap.", 'Content', 100, 350, color_white, TEXT_ALIGN_LEFT, TEXT_ALIGN_CENTER)
        ...
        draw.SimpleText("Explodes on death, dealing Poison damage. Does not explode when decapitated. Weak to headshots. On higher difficulties, it will explode when near a enemy.", 'Content', 100, 800, color_white, TEXT_ALIGN_LEFT, TEXT_ALIGN_CENTER)
        ...
        draw.SimpleText("Ranged attackers that spits flesh at enemies. Inflicts Bleeding. On higher difficulties, it will spit multiple projectiles.", 'Content', 100, 900, color_white, TEXT_ALIGN_LEFT, TEXT_ALIGN_CENTER)
        ...
        draw.SimpleText("Emits screaming shockwaves when approached. Deals Lightning damage. On higher difficulties, it will scream faster.", 'Content', 100, 1100, color_white, TEXT_ALIGN_LEFT, TEXT_ALIGN_CENTER)
        ...
        draw.SimpleText("Emits growling shockwaves when approached. Deals Cold damage. On higher difficulties, it will start out at Critical.", 'Content', 100, 1300, color_white, TEXT_ALIGN_LEFT, TEXT_ALIGN_CENTER)
        ...
        draw.SimpleText("Traducer", 'Heading', 50, 1650, color_white, TEXT_ALIGN_LEFT, TEXT_ALIGN_CENTER)
        local elite_next_pos = write_paragraph({
            "Extremely dangerous enemy with high health, agility, and armor. It can slowly heal, but this can be interrupted by shooting it.",
            "Has a chance to rage when attacked, increasing speed and attack speed.",
        }, 1700)
        draw_resistances(1650, {[HORDE.DMG_POISON] = 1.6, [HORDE.DMG_BLAST] = 1.2})
		
        draw.SimpleText("Sieger", 'Heading', 50, elite_next_pos + 50, color_white, TEXT_ALIGN_LEFT, TEXT_ALIGN_CENTER)
		elite_next_pos = write_paragraph({
            "Dangerous ranged enemy with armcannons that shoot out mortars. It's projectiles predict movement.",
            "Shooting it with > 15 damage will cause it to flee. Takes 50% damage from Dissolve type damage.",
        }, elite_next_pos + 120)
        draw_resistances(elite_next_pos - 150, {[HORDE.DMG_LIGHTNING] = 1.5})
		
        draw.SimpleText("Peacekeeper", 'Heading', 50, elite_next_pos + 50, color_white, TEXT_ALIGN_LEFT, TEXT_ALIGN_CENTER)
		elite_next_pos = write_paragraph({
            "Dangerous melee enemy with a sword. It can leap and enter a slash mode, which deals damage all around it.",
            "It can also slam dunk you, and has a block ability where it will take 0 damage. Takes 75% of all damage, but it is weak to damage on the head.",
        }, elite_next_pos + 120)
		
        draw.SimpleText("Wraith", 'Heading', 50, elite_next_pos + 50, color_white, TEXT_ALIGN_LEFT, TEXT_ALIGN_CENTER)
		elite_next_pos = write_paragraph({
            "Ranged special zombie that shoots projectiles. It can shoot a line of Wraith energy, and a Homing Extinugisher",
            "On higher difficulties, it will predict movement.",
        }, elite_next_pos + 120)
		
        draw.SimpleText("Deterrent", 'Heading', 50, elite_next_pos + 50, color_white, TEXT_ALIGN_LEFT, TEXT_ALIGN_CENTER)
		elite_next_pos = write_paragraph({
            "Dangerous zombie that shoots poisonous projectiles that inflict Break.",
            "On higher difficulties, It will predict movement. It can slowly heal, but this can be interrupted by shooting it.",
        }, elite_next_pos + 120)

        draw.SimpleText("Plague Elite", 'Heading', 50, elite_next_pos + 50, color_white, TEXT_ALIGN_LEFT, TEXT_ALIGN_CENTER)
		elite_next_pos = write_paragraph({
            "Dangerous enemy with high health. Can fire devastating energy balls and summon minions.",
        }, elite_next_pos + 120)

        draw.SimpleText("Bosses", 'LargeTitle', 50, 3050, color_white, TEXT_ALIGN_LEFT, TEXT_ALIGN_CENTER)

        draw.SimpleText("Alpha Gonome", 'Heading', 50, 3100, color_white, TEXT_ALIGN_LEFT, TEXT_ALIGN_CENTER)
        ...
        draw.SimpleText("Rekindled Behemoth", 'Heading', 50, next_pos + 50, color_white, TEXT_ALIGN_LEFT, TEXT_ALIGN_CENTER)
        next_pos = write_paragraph({
            "A behemoth with massive mind energy and armor, capable of wiping out both combine and human settlements.",
            "Claw Swipe: Deals massive Slashing damage and knocks the enemy back.",
            "Bullseye: Spawns 4 Bullseye minions that use hitscan attacks on the enemy's last position.",
            "DwarfSphere: Spawns a Dwarf Sphere that deals AOE fire damage whenever an enemy is near it, and can do a massive AOE attack that deals shock and fire damage near it.",
            "Earthshatter: Creates a tracing line of smoke by it's arm. Deals Blast Damage.",
            "Beam Attack: Spawns 2 massive beams based upon the X and Z coordinates of the enemy's position, which deals massive damage.",
			"Lifeseeker: Creates 4 homing projectiles that deal shock and decay upon impact.",
        }, next_pos + 100)
