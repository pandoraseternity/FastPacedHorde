...
function PANEL:Init()
    ...
    local function createBtn(text, panel, dock)
        ...
        panel.Paint = function(pnl, w, h)
            surface.SetDrawColor(116, 116, 116)
    ...
    for _, category in pairs(HORDE.categories) do
        ...
        for _, item in pairs(HORDE.items) do
            if item.category == category and ((item.whitelist == nil) or (item.whitelist and item.whitelist[class]) or (MySelf:Horde_GetCurrentSubclass() == "Gunslinger" and item.category == "Pistol")) and ((item.blacklist == nil)) then
				...
				--if (item.blacklist and item.blacklist[class]) then goto cont end
...
function PANEL:Paint(w, h)
    ...
    -- Entire 
    if ScrH() < 1080 then
        draw.RoundedBox(0, 0, 0, w, h, Color(121,105,105))
    else
