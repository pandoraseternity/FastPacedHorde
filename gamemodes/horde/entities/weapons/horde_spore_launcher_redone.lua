...
SWEP.PrintName = "Spore Launcher Redone"
SWEP.Category = "Horde"
SWEP.Spawnable= true
...
if SERVER then
local entity = ents.Create( "horde_spore_edit" )
...
phys:ApplyForceCenter( entity:GetForward() * 500 )
end
end )
end
end
...
if self.FiresUnderwater == false and self.Owner:WaterLevel() == 3 then return end
if SERVER then
local entity = ents.Create( "horde_spore_alt_edit" )
entity:SetOwner( self.Owner )
if IsValid( entity ) then
local Forward = self.Owner:EyeAngles():Forward()
local Right = self.Owner:EyeAngles():Right()
